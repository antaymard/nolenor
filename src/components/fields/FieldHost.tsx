...
import { resolveFieldVariant, type FieldSurface } from "@/../convex/config/fieldVariants";
...
// resolveFieldVariant), puis délègue au shell déclaré par le registry pour
// CE variant. Ajouter un variant = une entrée dans fieldRegistry, jamais une
// nouvelle branche ici.
export default function FieldHost({
  ...
  const options = placement.variantOptions;

  // Ne devrait jamais arriver (le registry et le catalogue fieldVariants
  // sont censés rester en phase) : dégrade en silence plutôt que de
  // planter le rendu de tout le node.
  if (!registration) return null;

  const common = { field, value, surface, options };
  ...
  switch (registration.shell) {
    ...
    case "custom": {
      ...
      return <Component {...common} onCommit={onCommit} onEscalate={onEscalate} />;
    }
    default: {
