...
1. `websearch` (`websearchTool`) — recherche web via Parallel API avec `search_effort` (`low`, `medium`, `high`). Retourne des résultats synthétisés.
2. `open_webpage` (`openWebPageTool`) — extraction de contenu de pages web/PDF publiques (max 10 URLs), avec focus par objectif.
