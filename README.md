...
npx convex env set VOYAGE_API_KEY       ...
...
| `R2_IMAGE_TRANSFORM` | — | Set to `cloudflare` to serve model-bound images through Cloudflare Image Transformations, so Nolë is sent a downscaled copy when it looks at an image. **How much this actually saves depends on the model's image encoder, and you should measure it before counting on it**: a continuous encoder (Anthropic-style, ~1380 tokens/megapixel) saves about half at 768 px, a 768 px tiled encoder (Gemini-style) up to 4×, and a 512 px tiled encoder (OpenAI-style) saves *nothing* — 768 px fills the same 2×2 tiles as 1024 px. Everything here is routed through OpenRouter, so the encoder is whatever it picked. Requires Transformations enabled on the R2 custom domain's zone — a metered Cloudflare feature; when it is not enabled, `/cdn-cgi/image/…` returns 404, which is why this is opt-in. Unset, full-size originals are sent. | Cloudflare → Images → Transformations |
| `R2_IMAGE_MAX_EDGE` | — | Longest edge, in pixels, of those downscaled copies. Defaults to `768`, the token-optimal point across the three encoder families above. Raise it when Nolë needs to judge fine visual detail rather than read text — the indexed `VISIBLE_TEXT` always ships alongside the image, so the pixels never have to carry the text. Ignored unless `R2_IMAGE_TRANSFORM` is set; an unparseable value falls back to the default. | — |
...
| `NEWSLETTER_SEGMENT_ID` | — | Resend segment new accounts are added to, once their email address is verified — immediately for Google, after the OTP for a password sign-up, never for an address that is never confirmed. Only accounts created after this shipped are eligible, and each is pushed to Resend at most once, so a password reset never re-subscribes someone who opted out. Needs `NEWSLETTER_TOPIC_ID` set too — either one missing and the subscription is skipped. Only runs on a production `SITE_URL`, so dev sign-ups never touch your contact list. Deleting an account deletes the contact. | Resend → Audiences → Segments |
| `NEWSLETTER_TOPIC_ID` | — | Resend topic those accounts are opted into (the product newsletter). Resend puts the unsubscribe link in every email sent to a topic. | Resend → Audiences → Topics |
